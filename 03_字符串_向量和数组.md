...
- 读写string对象(每次仅能读取被空格或换行符分隔的一个词)
  ...
  cin >> s;
  cout << s << endl;
  while (cin >> s) //  >>运算符会返回流参数，检测书否遇到文件结束标记或非法输入
...
## 2.3 处理string对象中的字符
cctype头文件中声明的函数见82页表3.3，如判断某个字符是否是字母
...
- 使用迭代器进行遍历
  ...
  for (auto it = s.begin(); it != s.end(); ++it)
    cout << *it << endl;
  ```
- 泛型编程：C++程序员一般是由迭代器而非下标，一般使用!=而非<以表示结束条件。原因是所有容器都支持迭代器和!=，而不是所有容器都支持下标和<
- 迭代器类型
  ```cpp
...
- 多个string对象可以直接通过 '+' 连接起来，而C风格字符串之间不可以这么做，只能调用 cstring 头文件中的strcat函数。(更多函数见P109)
